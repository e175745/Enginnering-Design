...
class FightingScene: GameSceneBase {
    ...
    private func start(){
        ...
        self.fishsize2 = Float(round(self.gameStatus.FishSize / 6))
        self.state = .fighting
    }
...
//            self.number = Int(floor((Double(self.gameStatus.FishRarity)+1)/2))
            ...
            case 1..<4:
                ...
                self.gameStatus.FishName = self.fishTypeList[self.FishTypeSmall]
                break
            case 4..<7:
                self.visualizer.makeFish(FishName: self.fishTypeList[self.FishTypeNormal])
                self.gameStatus.FishName = self.fishTypeList[self.FishTypeNormal]
                break
            case 7..<10:
                self.visualizer.makeFish(FishName: self.fishTypeList[self.FishTypeBig])
                self.gameStatus.FishName = self.fishTypeList[self.FishTypeBig]
                break
            case 10:
                self.visualizer.makeFish(FishName: self.fishTypeList[5])
                self.gameStatus.FishName = "taiyaki"
                break
            default:
                break
            }
